...
SAMPLES_PER_TYPE = 100 # 每个种类参与训练的数据量
LEARNING_RATE = 0.001
MODEL_PATH = "cnn/food101_convnet_model_2/" #保存模型的path
ALL = 10000 # 测试用的数据量
EPOCHS = None
UNITS = 101 # 此数据应该与参与训练的种类数量相同
STEPS = UNITS*SAMPLES_PER_TYPE*10

def cnn_model_fn(features, labels, mode):
    # RGB pic since color does matter for a food
    input_layer = tf.reshape(features["x"], [-1, 128, 128, 3,1])
    ...
    pool1 = tf.layers.max_pooling3d(inputs=conv1, pool_size=[2, 2, 3], strides=2)
    # 64 64 1 4
